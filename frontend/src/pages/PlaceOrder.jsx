      ...
      {/* left side ................. */}
        <div className='flex flex-col gap-4 w-full sm:max-w-[480px]'>
          ...
          <div className='flex gap-3'>
            ...
            <input className='border border-gray-300 py-1.5 px-3.5 rounded w-full' type='email' placeholder='Email address'/>
          <div className='flex gap-3'>
            <input className='border border-gray-300 py-1.5 px-3.5 rounded w-full' type='text' placeholder='City'/>
            <input className='border border-gray-300 py-1.5 px-3.5 rounded w-full' type='text' placeholder='State'/>
          </div>
          <div className='flex gap-3'>
            <input className='border border-gray-300 py-1.5 px-3.5 rounded w-full' type='number' placeholder='Zipcode'/>
        ...
        <div className='mt-8'>
            ...
            <div className='flex gap-3 flex-col lg:flex-row'>
              <div onClick={()=>setMethod("stripe")}  className='flex items-center gap-3 border p-2 px-3 cursor-pointer'>
                  <p className={`border-none min-w-3.5 h-3.5 border rounded-full ${method === "stripe" ? "bg-green-500" : "" }`}></p>
                  <img src={assets.stripe_logo} alt="" className='mx-5 h-4'/>
              </div>    
               <div onClick={()=>setMethod("razorpay")} className='flex items-center gap-3 border p-2 px-3 cursor-pointer'>
                  <p className={`border-none min-w-3.5 h-3.5 border rounded-full ${method === "razorpay" ? "bg-green-500" : "" }`}></p>
                  <img src={assets.razorpay_logo} alt="" className='mx-5 h-4'/>
              </div>  
               <div onClick={()=>setMethod("cod")} className='flex items-center gap-3 border p-2 px-3 cursor-pointer'>
                  <p className={`border-none min-w-3.5 h-3.5 border rounded-full ${method === "cod" ? "bg-green-500" : "" } `}></p>
